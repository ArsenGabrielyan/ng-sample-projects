...
export class CalcComponent {
  ...
  answer(){this.input = this.input;this.eq = 1;}
  getLastOperand(){
  ...
  plusmn(){if(this.disabled) return; this.input = `${+this.input * -1}`; this.eq = 1;}
  clear(){this.input = ""; this.disabled = false}
  del(){if(this.disabled) return; this.input = this.input.substring(0,this.input.length-1)}
}