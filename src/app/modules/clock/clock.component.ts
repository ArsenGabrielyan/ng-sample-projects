...
export class ClockComponent implements OnDestroy, OnInit, AfterViewInit {
  mode = localStorage.getItem("clock-mode");
  hrFormat = localStorage.getItem("clock-format");
  glowEnabled = localStorage.getItem("clock-glowEnabled");
  yearShown = localStorage.getItem("clock-yearShown");
  hr!: Observable<string>; 
  mn!: Observable<string>; 
  sc!: Observable<string>; 
  ...
  initToggler(){
    if(this.mode === "light") this.handleToggleDarkMode();
    if(this.hrFormat === "24") this.handleToggleFormat();
    if(this.glowEnabled === "no") this.handleToggleGlow();
    if(this.yearShown === "no") this.handleToggleYear();
  }
  handleToggleFormat(){
  ...
  handleToggleGlow(){
    this.toggleGlow = !this.toggleGlow;
    this.toggleGlow ? localStorage.setItem("clock-glowEnabled", "yes") : localStorage.setItem("glowEnabled", "no")
  }
  handleToggleYear(){
