...
export class PaintComponent implements AfterViewInit {
  @ViewChild("draw", {read: ElementRef<HTMLCanvasElement>, static: false}) 
  ...
  save(){
    ...
    const ext = prompt("select format (Supported: .jpg (.jpeg), .png, .tiff, .gif, .webp, .bmp)", ".jpg");
  ...
  respH(query:MediaQueryList){
    query.matches ? this.canvas.nativeElement.height = this.canvas.nativeElement.offsetHeight*4.75 : this.canvas.nativeElement.height = this.canvas.nativeElement.offsetHeight;
  }
}