...
export class WeatherComponent implements OnInit {
  weather_input = "";weather_city = "";
  weatherShown = false;
  weather_temp = 0;weather_windspd = 0;
  weather_humid = 0;weather_code = 0;
  current_weather!:string;
  constructor(private httpReq: HttpService, private other: OtherFeaturesService){}
  ngOnInit(): void {this.weatherShown = false;window.onkeydown = (e)=>{if(e.key === "Enter") this.searchWeather();}}
  getWeather(val:any){
  ...
  searchWeather(){if(this.weather_input.trim() === "") {alert("Enter City or Location");this.weather_input = ""} else this.httpReq.getWeatherDetails(this.weather_input).subscribe((val:any)=>val.pipe(map((res:any)=>this.getWeather(res)),finalize(()=>this.weather_input="")).subscribe());}
  ...
  getPlace(place:any){const {city,village,town,municipality, country, state, hamlet} = place.address, mentionedCommunity = city || village || town || municipality || state || hamlet;this.weather_city = `${mentionedCommunity || ""}${mentionedCommunity ? ", " : ""} ${country}`}
}